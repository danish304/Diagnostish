...
Многоуровневая отказоустойчивость. Безопасный парсинг данных (Parser) и безопасное выполнение WMI-запросов (ExecutorWmi) с раздельной обработкой ошибок доступа, таймаутов и повреждённых записей. Отсутствие или некорректность отдельных WMI-полей не прерывает сбор — утилита продолжает работу, фиксируя предупреждения (Warnings) и критические ошибки (CriticalErrors) в итоговом отчёте.
